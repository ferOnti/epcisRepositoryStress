...
function updateEpcidStep(epcList, po, eventId) {
	...
	var update   = {$set: {step: 5, eventId: eventId}}
...
function buildStagingPurchaseOrder(doc) {
	step = config.steps["5"]
	...
	if (Math.random() > step.poPercentage) {
		return new Promise(function (resolve, reject) { 
			resolve( sprintf("no staging products for purchase order %s", doc.po))
		})
	}
	var collection = global.db.collection('stress_products');
	var criteria = {bizTransaction: doc.po, step:3}
		...
		if (open) {
	     	cursor.next(function(err, item) {
				...
				if (item == null) {
					...
					buildStagingPurchaseOrder(item).then( function () {	
			...
			console.log("        staged products: %d" ,numItems)
    		iterateResolve()
    	}
    })	
}
...
function step5() {
	var db = global.db
	var step = config.steps["5"]

	var collection = db.collection('stress_purchase_orders');

	console.log("step 5: " +step.name)
	numItems = 0
	numPos = 0
