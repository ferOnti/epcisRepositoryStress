...
function updateItems(parentId, epcListArray) {
	var db = global.db
	var collection = db.collection('stress_products');
	var criteria = { parentIds : parentId  }
...
function buildReceiveCases(doc) {
	...
	sb = sb.replace("{parentID}", doc.case)
	...
	return new Promise(function (resolve, reject) { 
		...
		updateCases(doc.case)
			.then( () => {return updateItems(doc.case, doc.epcList)} )
			.then( function(res) {
				resolve()
			})
...
function step13() {
	...
	return new Promise(function (resolve, reject) { 
	    cursor = collection.find({step:12});
