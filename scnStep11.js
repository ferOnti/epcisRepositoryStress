...
function updateCases(caseListArray) {
	var db = global.db
...
function buildSendCases(docs) {
	...
	for (id in docs) {
		...
		caseListArray.push(docs[id].case)
	}
		...
		if (open) {
	     	cursor.next(function(err, item) {
				...
				if (item == null) {
					...
					if (casesBuffer.length +1 > step.maxCasesToSend) {
						buildSendCases(casesBuffer).then( function () {	
							casesBuffer = []
...
function step9() {
	...
	return new Promise(function (resolve, reject) { 
	    cursor = collection.find({step:10});
